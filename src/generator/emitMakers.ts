...
function renderMaker(name: string): string {
  return `  const errors: ValidationIssue[] = [];
...
export function emitSchemaMakers(
  ...
  files.set('validators.ts', renderer.render(model, config));

  return files;
}

export function renderEntityMakers(model: OpenApiModel): string {
  return model.schemas
    .map(({ name }) => {
      return `export function make${name}(input: unknown): Result<ShapeOf${name}> {\n${renderMaker(name)}}`;
    })
    .join('\n\n');
}
