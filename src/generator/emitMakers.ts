...
class ValidatorRenderer {
  ...
  render(model: OpenApiModel, runtimeImport: string): string {
    ...
    return `/**
      ...
      */
      ...
      import type { ValidationIssue, ValidationPath } from ${JSON.stringify(runtimeImport)};
      import type { ${typeNames} } from '../quickpay-api';
...
function renderMaker(name: string, runtimeImport: string): string {
  return `/**
 * Generated by openapi-contract-kit.
 * Do not edit directly.
 */
...
export function emitSchemaMakers(
  ...
  files.set(
    ...
    renderer.render(model, config.runtimeImport)
  );
  for (const { name } of model.schemas) {
    files.set(`schemas/${name}.ts`, renderMaker(name, config.runtimeImport));
  }
